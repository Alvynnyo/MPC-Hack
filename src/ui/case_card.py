...
Palette : bleu sombre (critique) · jaune clair (avertissement) · vert (ok).
...
def render_controls() -> str:
    """Le bloc boutons + raccourcis. Partagé entre la vue simple et le deck."""
    return f"""
    <div class="actions">
...
# --- Rendu d'une carte (header + corps), footer optionnel ------------------ #

def render_card_inner(case: CaseFile, footer: str = "") -> str:
    """Renvoie l'élément .card (header + corps). `footer` est inséré dans .card
    s'il est fourni (vue simple) ; le deck le laisse vide et gère ses contrôles
    séparément."""
    ...
    <div class="body">
      ...
      <div class="metrics">
        ...
        <div class="metric" style="grid-column: 1 / -1;">
          ...
          <div class="score-track"><div class="score-fill" style="width: {score_pct}%; background: {risk['bar']};"></div></div>
        </div>
      </div>

      <p class="section">Analyse IA</p>
      <div class="verdict"><div class="verdict-text">{case.verdict}</div></div>

      <p class="section">Signaux détectés</p>
      <div class="evidence">{evidence_html}</div>

      <p class="section">Historique récent de la carte</p>
      <div class="timeline">{previous_html}</div>
...
# --- Feuille de style statique (partagée) ---------------------------------- #
...
.case-id { font-family: var(--font-mono); font-size: 15px; font-weight: 600; color: var(--c-text); }
...
.metric { border: 1px solid var(--c-border); border-radius: 10px; padding: 12px 14px; background: #FCFCFD; }
.metric-label {
...
.score-track { height: 6px; border-radius: 999px; background: #EAECF0; margin-top: 8px; overflow: hidden; }
.score-fill { height: 100%; border-radius: 999px; }

.section {
  font-size: 11px; font-weight: 600; letter-spacing: 0.06em;
  text-transform: uppercase; color: var(--c-text-3); margin: 0 0 10px;
}
.verdict {
  background: #F9FAFB; border: 1px solid var(--c-border);
  border-left: 3px solid #1E40AF; border-radius: 8px;
  padding: 14px 16px; margin-bottom: 22px;
}
.verdict-text { font-size: 14px; line-height: 1.6; color: var(--c-text); }

.evidence { margin-bottom: 22px; }
.ev-row {
  display: grid; grid-template-columns: 20px 96px 1fr auto; gap: 12px;
...
.timeline { border: 1px solid var(--c-border); border-radius: 10px; overflow: hidden; }
.tx-row {
  display: grid; grid-template-columns: 84px 1fr auto 64px; gap: 10px;
...
.footer { padding: 16px 24px 20px; border-top: 1px solid var(--c-border); background: #FCFCFD; }
.actions { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-bottom: 14px; }
.btn {
  display: inline-flex; align-items: center; justify-content: center; gap: 8px;
  font-family: var(--font-sans); font-size: 13px; font-weight: 600;
  padding: 10px 12px; border-radius: 9px; border: 1px solid var(--c-border-strong);
...
.shortcuts {
  ...
  gap: 10px; font-size: 12px; color: var(--c-text-3);
}
...
# --- Document complet pour une carte seule (vue statique / fallback) ------- #

def render_case_card(case: CaseFile) -> str:
    """Document HTML complet affichant une seule carte (avec footer)."""
    inner = render_card_inner(case, footer=render_controls())
    return f"""<!DOCTYPE html>
<html>
<head><style>{CARD_CSS}
body {{ padding: 16px; display: flex; justify-content: center; align-items: flex-start; }}
