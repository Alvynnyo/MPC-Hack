...
# L'import est déjà présent comme demandé
from src.detection.layer4_cross_card import get_cross_card_transactions
...
def initialize_fraud_queue(
    ...
    for i, ctx in enumerate(contexts):
        ...
        # ── MODIFICATION ICI : Utilisation de get_cross_card_transactions ──
        if signal == "cross_card" and anchor_ts is not None:
            # On va chercher précisément les cartes concurrentes sur la même fenêtre temporelle chez le même marchand
            cross_df = get_cross_card_transactions(
                df=df_full,
                merchant_name=ctx.merchant_name,
                anchor_timestamp=anchor_ts,
                current_card_id=ctx.card_id,
                window_hours=2
            )
            
            # On peuple les données de périphérique avec ces transactions synchronisées
            case.device_data = [
                {
                    "last_seen": pd.to_datetime(n_row["timestamp"]).strftime("%m-%d %H:%M"),
                    "card_id": str(n_row["card_id"]),
                    "tx_count": int(len(df_full[df_full["card_id"] == n_row["card_id"]])),
                    "amount": float(n_row["amount"]),
                    "merchant": str(n_row["merchant_name"]),
                }
                for _, n_row in cross_df.head(10).iterrows()
            ]
        else:
            # Fallback historique classique (Appareil / IP) si le signal dominant n'est pas la fraude croisée pure
            current_device = row_flagged.get('device_id')
            current_ip = row_flagged.get('ip_address')
            network_frames = []

            if pd.notna(current_device):
                network_frames.append(df_full[(df_full['device_id'] == current_device) & (df_full['card_id'] != ctx.card_id)])
            if pd.notna(current_ip):
                network_frames.append(df_full[(df_full['ip_address'] == current_ip) & (df_full['card_id'] != ctx.card_id)])

            if network_frames:
                network_txs = pd.concat(network_frames).drop_duplicates(subset=["transaction_id"]).sort_values(by="timestamp", ascending=False).head(10)
                case.device_data = [
                    {
                        "last_seen": pd.to_datetime(n_row["timestamp"]).strftime("%m-%d %H:%M"),
                        "card_id": str(n_row["card_id"]),
